...
// Silencing warnings due to
// https://github.com/OpenZeppelin/openzeppelin-upgrades/issues/369#issuecomment-920399254
upgrades.silenceWarnings();

describe('ERC721AUUPSUpgradeable', function () {
  beforeEach(async function () {
    this.ERC721A = await ethers.getContractFactory('ERC721AUUPSUpgradeableMock');
    this.ERC721Receiver = await ethers.getContractFactory('ERC721ReceiverMock');
    this.erc721a = await upgrades.deployProxy(this.ERC721A, ['Azuki', 'AZUKI'], { kind: 'uups' });
  });
  ...
  context('upgrade', async function () {
    it('succesfully upgrade to V2', async function () {
      this.ERC721A = await ethers.getContractFactory('ERC721AUUPSUpgradeableV2Mock');
      this.erc721a = await upgrades.upgradeProxy(this.erc721a.address, this.ERC721A);
      expect(await this.erc721a.numberMinted(ZERO_ADDRESS)).to.equal(42);
    });
  });
});
